document.addEventListener("DOMContentLoaded", () => {
  ...
  const cart = document.getElementById("cart"); // agar tumhari cart ka div h
  ...
  openCartBtn?.addEventListener("click", () => {
    cart.classList.toggle("active"); // apni cart ke liye CSS mein .active lagana
  });
});
...
// === Cart Drawer ===
const LS_KEY = "specialty_cart";  // unique key for specialty drinks cart
...
function currency(n){ return "Rs " + Number(n).toFixed(2); }
...
function renderCart(){
  ...
  listEl.innerHTML = cart.map(item => `
    <li class="cart-item">
      ...
      <button class="remove" data-id="${item.id}"><i class="fa fa-trash"></i></button>
    </li>
